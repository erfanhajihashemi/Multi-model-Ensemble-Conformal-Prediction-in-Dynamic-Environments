...
def create_model(dataset, model_name="resnet18", device=torch.device("cpu"), **kwargs):
    ...
    if model_name == "resnet18":
        ...
        model = torchvision.models.googlenet(weights="DEFAULT", **kwargs)
        model.fc = nn.Linear(1024, dataset.n_class)


    elif model_name == "mobilenet_v2":
        model = torchvision.models.mobilenet_v2(weights="DEFAULT", **kwargs)
        model.classifier[1] = nn.Linear(model.classifier[1].in_features, dataset.n_class)
        ...
        model = torchvision.models.efficientnet_b0(weights="DEFAULT", **kwargs)
        if not isinstance(dataset, ImageNet):
            model.classifier[1] = nn.Linear(model.classifier[1].in_features, dataset.n_class)
